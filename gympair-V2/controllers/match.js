...
router.get('/', [verifyToken], async function(req, res) {
    ...
    if (req.query && Object.keys(req.query).length > 0) {
        ...
        if(req.query.gender!=='all'&&req.query.vaccinated!=='both'){
          ...
          query={ $and: [{ gender: req.query.gender }, { vaccinated: vaccinated }] }  
        ...
        User.find(query)
            ...
            .select('_id firstName lastName username gender profile_picture date_of_birth gym vaccinated  weight typeWeight height typeHeight')
            .populate({
                path: 'gym',
                match: locationQuery
            })
                    ...
                    if (users && users.length > 0) {
                        ...
                        for (; i < ageLength; i++) {
                            (function(i) {
                                ...
                                if (age >= req.query['age_start'] && age <= req.query['age_end']) {
                                        ...
                                        excludeBlocklist(matched, async function(e, arr) {
                                            if (e) {
                                                ...
                                                if(loggedInUser.gym.length > 0) {
                                                    gymUsers = await getUsersByGymTypeAndLocation(req.userId);

                                                    if (gymUsers&&gymUsers.length > 0) {
                                                        const commonData = uniqueArray.filter((obj1) =>
                                                            gymUsers.some((obj2) => obj2._id.toString() === obj1._id.toString())
                                                        );
                                                ...
                                                if(loggedInUser.gym.length > 0) {
                                                    ...
                                                    if (gymUsers.length > 0) {
                                                        const commonData = gymUsers.filter((obj1) =>
                                                            uniqueArray.some((obj2) => obj2._id.toString() === obj1._id.toString())
                                                        );
                                                        return res.json({ status: true, data: commonData });
                                                    }
                                                }
    ...
    async function getUsersByGymTypeAndLocation(loggedInUserId) {
      try {
        ...
        const loggedInUser = await User.findById(loggedInUserId).populate('gym').select('_id firstName lastName username gender profile_picture date_of_birth gym vaccinated  weight typeWeight height typeHeight');
        ...
        if (type === 0) {
          ...
          const users = await User.find({ gym: { $in: [_id] }, _id: { $ne: loggedInUser._id } }).populate('gym').select('_id firstName lastName username gender profile_picture date_of_birth gym vaccinated  weight typeWeight height typeHeight');
          ...
          const users = await User.find({}).populate('gym').select('_id firstName lastName username gender profile_picture date_of_birth gym vaccinated  weight typeWeight height typeHeight');
          const finalUsers = [];
          for(let user of users) {
            ...
            if (user.gym.length > 0){
              ...
              let checkDistance = isWithinRadius(userGym.latitude, userGym.longitude, latitude, longitude, radius);
    ...
    function isWithinRadius(userLatitude, userLongitude, gymLatitude, gymLongitude, radius) {
      ...
      const centralAngle = 2 * Math.asin(Math.sqrt(Math.sin(latDiff / 2) ** 2 + Math.cos(userLatRadians) * Math.cos(gymLatRadians) * Math.sin(lngDiff / 2) ** 2));
        ...
        User.findOne({ _id: req.userId }, 'gym age_start age_end')
            ...
            .exec(function(err, user) {
                if (err) {
                    ...
                    if ((!user.gym || (user.gym && user.gym.length == 0)) && !user.age_start && !user.age_end) {
                        finalCallback(null, { message: "Please set your preferences before finding a match." });
                    } else {
                        cb(null, user);
                    }
                }
    ...
    function excludeBlocklist(matched, cb) {
        User.findOne({ _id: req.userId }, 'blocklist')
            .exec(function(e, list) {
                if (e) {
                    console.log(e);
                } else {
                    if (list.blocklist && list.blocklist.length > 0) {
                        var filtered = matched.filter(obj => !list.blocklist.includes(obj._id));
                        cb(null, filtered);
                    } else {
                        cb(null, matched);
                    }
    ...
    Friend.findOne({
        ...
        if (err) {
            ...
            if (fr && fr.status == 'accepted') { 
                res.status(200).json({ status: false, message: "Already in Friend list" });
            } else if (fr && fr.status == 'pending') {
                res.status(200).json({ status: false, message: "Request sent already" });
            } else {
                User.findOne({ _id: req.userId }, "subscription subscriptionExpiry").exec(function (
                    ...
                    if (e) {
                      ...
                      if(user && user.subscription === 1 && user.subscriptionExpiry > currentDate.getTime()){
                        var request = { user1: req.userId, user2: req.params.id, status: 'pending', request_by: req.userId };
                        Friend.create(request, function(er, re) {
                            if (er) {
                                res.status(500).json({ status: false, message: er });
                            } else {
                                sendPushNotification(req.params.id, "Pair Request", " New pair request");
                                saveNotification(9, req.userId, req.params.id,re['_id']);
                                res.status(200).json({ status: true, message: "Request sent successfully.Please wait for response." });
                            }
                        })
                      }else{
                        ...
                        Friend.countDocuments({
                            ...
                            if (err) {
                                ...
                                res.status(200).json({ status: false, message: "Friend request limit reached for today." });
                            } else {
...
router.get('/respond_request/:id', [verifyToken], function(req, res) {
    Friend.findOne({ _id: req.params.id }).exec(function(err, fr) {
        if (err) {
            ...
            if (fr && fr.status !== 'pending') { //if already responded
                res.status(200).json({ status: false, message: "Request already responded" });
            } else if (fr && fr.status == 'pending') {
                ...
                Friend.updateOne({ _id: fr._id }, { status: st }, function(er, req) {
                    if (er) {
                        ...
                        sendPushNotification(user, "Pair Request Update", " Pair Request Responded.");
                        res.status(200).json({ status: true, message: "Pair Request responded successfully." });
                    }
                })
            } else if (!fr) {
                res.status(404).json({ status: false, message: "Request record not found" });
            }
        }
    });
});

router.get('/respond_request/status/:user_id', [verifyToken], function(req, res) {
    Friend.findOne({
        $or: [
                ...
                .exec(function(err, chat) {
                    if (err) {
                        ...
                        if (chat) {
                            ...
                            res.status(200).json({ status: true, message: "Chat found", data: chatObj});
                        } else {
                            ...
                            newChat.save(function(err, savedChat) {
                                if (err) {
                                    ...
                                    Chat.populate(savedChat, [
                                        { path: "user1", select: "_id firstName lastName username profile_picture" },
                                        { path: "user2", select: "_id firstName lastName username profile_picture" }
                                    ], function(populateErr, populatedChat) {
                                        if (populateErr) {
                                            res.status(500).json({ status: false, message: populateErr });
                                        } else {
                                            let chatObj = {
                                            ...
                                            res.status(200).json({ status: true, message: "New chat created", data: chatObj });
                                        }
                                    });
                                }
                            });
...
router.delete('/request/:id', [verifyToken], function(req, res) {
    Friend.deleteOne({ _id: req.params.id }).exec(function(err, fr) {
        if (err) {
            ...
            //console.log(fr);
            res.status(200).json({ status: true, message: "Request removed successfully" })
        }
    });
});
...
router.get('/follow/:id', [verifyToken], function(req, res) {
    User.findOne({ _id: req.userId }, '_id followings followers username')
        .exec(function(e, user) {
            if (e) {
                ...
                if (!user.followings || user.followings.length <= 0) {
                    ...
                    if (user.followings.includes(req.params.id)) { //user already follows other user
                        console.log("already following");
                        user.followings = user.followings.filter(x => !x.equals(req.params.id));
                        user.save();
    ...
    function addFollower(id, userId) {
        ...
        User.findOne({ _id: req.params.id }, 'followings followers username')
            .exec(function(e, user) {
                if (e) {
                    res.status(500).json({ status: false, message: e });
                } else {
    ...
    function removeFollowing(id, userId) {
        ...
        User.findOne({ _id: id }, 'followings followers')
            .exec(function(e, user) {
                if (e) {
                    res.status(500).json({ status: false, message: e });
                } else {
...
router.get('/like/:id', [verifyToken], function(req, res) {
    User.findOne({ _id: req.userId }, 'liked_to disliked_to username')
        .exec(function(e, user) {
            if (e) {
                res.status(500).json({ status: false, message: e });
            } else {

                if (!user.liked_to.includes(req.params.id)) { //already not liked
                    console.log("liked");
                    if (user.liked_to && user.liked_to.length > 0) { //liked list already
                        user.liked_to.push(req.params.id);
                        user.rewind = { action: 'like', user: req.params.id };
                        user.save();
                        sendPushNotification(req.params.id, "Like Update", user['username'] + " Likes you");
                    } else if (user.liked_to.length <= 0) { //no liked list already
                    ...
                    if (user.disliked_to.includes(req.params.id)) { //if user is in disliked list
                        user.disliked_to = user.disliked_to.filter(x => !x.equals(req.params.id));
                        user.save();
                    }
                    User.findOne({ _id: req.params.id }, 'liked_by disliked_by username')
                        .exec(function(err, usr) {
                            if (err) {
                                res.status(500).json({ status: false, message: e });
                            } else {
                                if (usr.liked_by && usr.liked_by.length > 0) { //liked list already
                            ...
                            if (usr.disliked_by.includes(req.userId)) { //if user is in disliked list
                                usr.disliked_by = usr.disliked_by.filter(x => !x.equals(req.userId));
                                usr.save();
                            }
                            sendPushNotification(req.params.id, "Like Update", user['username'] + " Likes you");
        ...
        .exec(function(e, user) {
            if (e) {
                ...
                if (!user.liked_to.includes(req.params.id)) { //already not liked
                    ...
                    user.save();
                    User.findOne({ _id: req.params.id }, 'liked_by disliked_by')
                        .exec(function(err, usr) {
                            if (err) {
                                res.status(500).json({ status: false, message: e });
                            } else { //unliked
...
router.get('/dislike/:id', [verifyToken], function(req, res) {
    User.findOne({ _id: req.userId }, 'liked_to disliked_to')
        .exec(function(e, user) {
            if (e) {
                res.status(500).json({ status: false, message: e });
            } else {
                if (!user.disliked_to.includes(req.params.id)) { //already not disliked
                    console.log("disliked");
                    if (user.disliked_to && user.disliked_to.length > 0) { //disliked list already
                        user.disliked_to.push(req.params.id);
                        user.rewind = { action: 'dislike', user: req.params.id };
                        user.save();
                    } else if (user.disliked_to.length <= 0) { //no disliked list already
                        user.disliked_to = [req.params.id];
                        user.rewind = { action: 'dislike', user: req.params.id };
                        user.save();
                    }
                    if (user.liked_to.includes(req.params.id)) { //if user is in liked list
                        user.liked_to = user.liked_to.filter(x => !x.equals(req.params.id));
                        user.save();
                    }
                    User.findOne({ _id: req.params.id }, 'liked_by disliked_by')
                        .exec(function(err, usr) {
                            if (err) {
                                ...
                                if (usr.disliked_by && usr.disliked_by.length > 0) { //disliked list already
                            ...
                            if (usr.liked_by.includes(req.userId)) { //if user is in liked list
                                usr.liked_by = usr.liked_by.filter(x => !x.equals(req.userId));
                                usr.save();
                            }
                            return res.status(200).json({ status: true })
        ...
        .exec(function(e, user) {
            if (e) {
                ...
                if (!user.rewind || !user.rewind.action) {
                    return res.status(400).json({ status: false, message: "Nothing to rewind." });
                } else {
                    ...
                    if (user.rewind.action == 'unlike') {
                        ...
                        if (user.disliked_to.includes(user2)) { //if user is in disliked list
                            user.disliked_to = user.disliked_to.filter(x => !x.equals(user2));
                            user.save();
                        }
                        User.findOne({ _id: user2 }, 'liked_by disliked_by')
                            .exec(function(err, usr) {
                                if (err) {
                                    res.status(500).json({ status: false, message: e });
                                } else {
                        ...
                        if (user.liked_to.includes(user2)) { //if user is in liked list
                            user.liked_to = user.liked_to.filter(x => !x.equals(user2));
                            user.save();
                        }
                        User.findOne({ _id: user2 }, 'liked_by disliked_by')
        ...
        .exec(function(err, user) {
            if (err) {
                ...
                if (user.blocklist && user.blocklist.length > 0) {
                    user.blocklist.includes(req.params.id) ? user.blocklist : user.blocklist.push(req.params.id);
                    user.followings = (user.followings && user.followings.length > 0) ? user.followings.filter(n => !n.equals(req.params.id)) : user.followings;
                    //console.log(user.followers.filter(n => !n.equals(req.params.id)));
                    user.followers = (user.followers && user.followers.length > 0) ? user.followers.filter(n => !n.equals(req.params.id)) : user.followers;
                    user.save();
                    res.status(200).json({ status: true, message: "User is added in blocklist." })
                } else {
...
var saveNotification = function(type, sender, receiver,requestId) {
    User.findOne({ _id: sender }, 'username firstName lastName')
        .exec(function(e, u) {
            if (e) {
                console.log(e);
            } else {
