...
var Storage = multer.diskStorage({ //multers disk storage settings
    ...
    filename: function(req, file, cb) {
        ...
        var imgName = "dp_" + file.originalname.split('.')[0] + '-' + seq + '.' + file.originalname.split('.')[file.originalname.split('.').length - 1];
...
router.post('/register', function(req, res) {
    ...
    upload(req, res, function(e, f) {
        ...
        const user = new User({
            ...
            "typeHeight": req.body.height && req.body.typeHeight ? req.body.typeHeight : 'cm',
            "typeWeight": req.body.weight && req.body.typeWeight ? req.body.typeWeight : 'kg',
            ...
            */
            "profile_picture": req.file ? config.apiUrl + "/uploads/" + req.file.filename : '',
        ...
        user.save((err, usr) => {
            if (err) {
                ...
                var OTPtext = "Your Gympair verification OTP is: " + seq + "\n" +
                ...
                request(url, (error, response, body) => {
                    if (error) {
                        ...
                        res.status(200).json({
                            status: true,
                            message: "User created successfully.We have sent OTP to your mobile Number.",
                            token: token
                        })
                    };
                });
                ...
                if (err) {
                    ...
                    if (user) {
                        if (user.otp && user.otp !== 0) {
                            ...
                            user['otp_created_at'] = dt.setHours(dt.getHours() + 1);
                            user.save();
                            var OTPtext = "Welcome to gympair. Your Gympair verification OTP is: " + seq + "\n" +
                                "Do not share this with anyone. ";
                            var url = 'https://mx.fortdigital.net/http/send-message?username=89180134&password=wkzb8uqn&to=' + req.body.phoneNumber + '&message=' + OTPtext;
                            ...
                            User.updateOne({_id:user['_id']},{$set:{last_login:lastDate}})
                        ...
                        return res.status(404).send({ message: "Unauthorized.User Doesn't exist." });
                    }
                }
            });
    } 
                ...
                if (err) {
                    ...
                    if (user) {
                        ...
                        if (user.otp && user.otp !== 0) {
                            ...
                            var OTPtext = "Welcome to gympair. Your Gympair verification OTP is: " + seq + "\n" +
                                "Do not share this with anyone. ";
                            var url = 'https://mx.fortdigital.net/http/send-message?username=89180134&password=wkzb8uqn&to=' + user.phoneNumber + '&message=' + OTPtext;
            ...
            if (err) {
                ...
                if (usr) {
                    if (usr.otp_created_at > dt.getHours()) {
                        ...
                        sendEmail({ email: usr['email'], name: usr['firstName']+" "+usr['lastName']});
                        res.status(200).json({
                            status: true,
                            message: "User Logged in successfully.",
                            token: token,
                        ...
                        res.status(401).send({ status: false, code: 500, message: "OTP expired.try again" });
                        return;
                    }
                } else {
                    res.status(401).send({ status: false, code: 500, message: "wrong OTP.Please Try again" });
                    return;
                }
            }
        });
...
router.post('/OTP', function(req, res) {
    ...
    User.findOne({ phoneNumber: req.body.phone })
        .select('_id firstName lastName username email gender phoneNumber otp otp_created_at')
        .exec(function(err, user) {
            if (err) {
                res.status(500).send({ status: false, code: 500, message: err });
                return;
                ...
                user['otp_created_at'] = dt.setHours(dt.getHours() + 1);
                user.save();*/
                User.updateOne({ _id: user['_id'] }, { otp: seq, otp_created_at: dt.setHours(dt.getHours() + 1) }, {
                    runValidators: true,
                    new: true,
                }, function(e, u) {
                    if (e) {
                        ...
                        var OTPtext = "Welcome to gympair. Your Gympair verification OTP is: " + seq + "\n" +
                            "Do not share this with anyone. ";
                        var url = 'https://mx.fortdigital.net/http/send-message?username=89180134&password=wkzb8uqn&to=' + req.body.phone + '&message=' + OTPtext;
    ...
    .exec(function(err,user){
      if (err) {
        ...
        var seq = Math.floor(Math.random() * 900000);
          ...
          user['otp_created_at']=dt.setHours(dt.getHours()+1);
          user.save();
            var OTPtext="Welcome to gympair. Your OTP is: "+seq+"\n"+
                          "Do not share this with anyone. ";
            var url = 'https://mx.fortdigital.net/http/send-message?username=89180134&password=wkzb8uqn&to='+req.body.phoneNumber+'&message='+OTPtext;
...
router.get('/forgot-password', function(req, res) {
    ...
    if (!phone) {
        res.status(500).send({ status: false, code: 500, message: "Please Provide Phone No." });
    } else {
        User.findOne({ phoneNumber: phone })
            .select('firstName lastName username email gender phoneNumber otp password_otp password_otp_created_at')
            .exec(function(err, user) {
                if (err) {
                    res.status(500).send({ status: false, code: 500, message: err });
                    return;
                    ...
                    if (!user) {
                        res.status(500).send({ status: false, code: 500, message: "Phone No. isnt registered with us." });
                    } else {
                        var minm = 100000;
                        var maxm = 999999;
                        var seq = Math.floor(Math
                        ...
                        user.save();
                        var OTPtext = "Your password reset OTP is: " + seq + "\n" +
                            "Do not share this with anyone. ";
                        var url = 'https://mx.fortdigital.net/http/send-message?username=89180134&password=wkzb8uqn&to=' + user.phoneNumber + '&message=' + OTPtext;
                        ...
                        res.status(200).json({
                            status: true,
                            message: "We have sent OTP for password to your registered mobile Number."
                            //token:token
                        })
                    }
                }
...
router.post('/reset-password', function(req, res) {
    ...
    if (!req.body.newPassword) {
        res.status(500).send({ status: false, code: 500, message: "Please provide new password;" });
    } else {
        User.findOne({ password_otp: req.body.otp })
            .select('firstName lastName username email gender phoneNumber password_otp password_otp_created_at')
            .exec(function(err, user) {
                if (err) {
                    res.status(500).send({ status: false, code: 500, message: err });
                    return;
                    ...
                    if (user) {
                        ...
                        if (user.password_otp !== 0) {
                            ...
                            res.status(400).send({ status: false, code: 400, message: "OTP is wrong or expired.Please try again" });
                        }
                    } else {
                        res.status(400).send({ status: false, code: 400, message: "OTP is wrong.Please try again" });
                    }
                }
            });
    }
...
router.get('/forgot-password', function(req, res) {
    ...
    if (!email) {
        res.status(500).send({ status: false, code: 500, message: "Please Provide Email." });
    } else {
        User.findOne({ email: email })
            ...
            .exec(function(err, user) {
                if (err) {
                    ...
                    if (!user) {
                        res.status(500).send({ status: false, code: 500, message: "Email isnt registered with us." });
                    } else {
                        var seq = Math.floor(Math.random() * 900000);
                        var dt = new Date();
                        user['password_otp'] = seq;
                        user['password_otp_created_at'] = dt.setHours(dt.getHours() + 1);
                        user.save();
                        var OTPtext = "Welcome to gympair. Your password reset OTP is: " + seq + "\n" +
                            "Do not share this with anyone. ";
                        var url = 'https://mx.fortdigital.net/http/send-message?username=89180134&password=wkzb8uqn&to=' + user.phoneNumber + '&message=' + OTPtext;
...
router.post('/reset-password', function(req, res) {
    ...
    if (!req.body.newPassword) {
        ...
        User.findOne({ email: req.body.email })
            .select('firstName lastName username email gender phoneNumber password_otp')
            .exec(function(err, user) {
                if (err) {
                    res.status(500).send({ status: false, code: 500, message: err });
                    return;
                    ...
                    if (user) {
                        ...
                        res.status(400).send({ status: false, code: 400, message: "Email not exits or invalid" });
                    }
                }
            });
    }
...
router.post('/change_email', [verifyToken], function(req, res) {
    ...
    if (!req.body.newEmail) {
        res.status(500).send({ status: false, code: 500, message: "Please provide new email you want to register." });
    } else {
        User.updateOne({ _id: req.userId }, { email: req.body.newEmail }, {
            runValidators: true,
            new: true,
        }, function(e, u) {
            if (e) {
    ...
    User.findOne({ _id: req.userId })
        .exec(function(err, usr) {
            if (err) {
                ...
                bcrypt.compare(req.body.oldPassword, usr.password, function(error, isMatch) {
                    if (error) {
                        res.status(500).send({ status: false, code: 500, message: error });
                        return;
                    } else if (isMatch) {
                        ...
                        res.status(500).send({ status: false, message: "Password not matched." });
                    }
                })
            }
        });
            ...
            .exec(function(err, gym) {
                if (err) {
                    ...
                    if (gym && gym._id) {
                        ...
                        User.updateOne({ _id: user }, gymData, {
                            runValidators: true,
                            new: true,
                        }, function(e, u) {
                            if (e) {
                        ...
                        if (data.title) {
                            ...
                            const gymm = new Gym({
                                ...
                                longitude: data.station_longitude&&data.station_longitude!=='null' ? data.station_longitude : null,
                                latitude: data.station_latitude&& data.station_latitude!=='null' ? data.station_latitude : null,
                                type: 1
                            });
                            gymm.save();
                            var gymData = {
...
var handleReferal = function(code) {
    User.findOne({ referal_code: code })
        .exec(function(err, user) {
            if (err) {
                console.log(err);
            } else {
                if(user&&user['_id']){
                    ...
                    Wallet.findOne({ user: user['_id'] }).exec(async function (err, wallet) {
                        if (err) {
                            console.log(err);
            }else{
...
var sendEmail = function(recvr) {
  ...
  function base64_encode(file) {
    ...
    const mailOptions = {
        ...
        subject: "Welcome to Gympair - Your Fitness Community Awaits!",
        attachments: [{
          filename: 'icon.png',
          content: base64str,
          type: "image/png",
        ...
        html: "<strong>Dear " + recvr['name'] + ",</strong><br>" + "\n" +
            "<br>"+
            
            "We extend a warm welcome to Gympair! As a valued member of our community, we're thrilled to have you on board and are eager to witness the incredible progress and accomplishments you'll achieve on your fitness journey.<br>" +
            "<br>"+
            " At Gympair, we are a passionate startup dedicated to revolutionizing the way you connect with like-minded individuals who share your enthusiasm for fitness. We understand that every step towards a healthier lifestyle matters, and we're committed to enhancing your user experience, providing you with a seamless interface, and delivering valuable benefits along the way.<br>" +
            "<br>"+
            "Our app has been thoughtfully designed to offer you easy and convenient access to a diverse community of gym-goers in your area. With Gympair, finding a gym buddy who shares your workout preferences or frequents the same gym has never been easier. Embrace the opportunity to connect, motivate, and inspire one another as you embark on your fitness endeavors together.<br>" +
            "<br>"+
            " But that's not all! Gympair goes beyond connecting you with fellow fitness enthusiasts. Our app also empowers you to track your progress effectively, enabling you to achieve your targets and goals. Utilize our range of calculators, and upcoming features like exercise demonstrations, nutrition tips  personalized workout plans to maximize your fitness journey.<br>" +
            "<br>"+
            " As a startup, we greatly value your patience and understanding as we continually strive to improve and enhance your Gympair experience. Your feedback and suggestions are essential to us, so please don't hesitate to reach out to our dedicated customer support team at helpme@gympair.com should you have any questions or need assistance getting started.<br>" +
            "<br>"+
            " Once again, welcome to Gympair! We are excited to accompany and support you on your fitness journey. Together, let's build a vibrant community that fosters motivation, inspiration, and success.<br>" +
            "<br>"+
            "Sincerely,<br>" +
            "Gympair Team<br>" +
            "Find Your Pair<br>"+
