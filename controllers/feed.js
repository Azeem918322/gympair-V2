...
var storage = multer.diskStorage({
    ...
    filename: function(req, file, callback) {
        ...
        callback(null, file.originalname.split('.')[0] + '-' + req.userId + '.' + file.originalname.split('.')[file.originalname.split('.').length - 1]);
    }
});
        ...
        if (req.files) {
            if (req.files['image']) {
                ...
                req.files['image'].forEach((f) => {
                    img.push(config.apiUrl + "/uploads/images/" + req.userId + "/" + f.filename)
                })
                post['image'] = img;
            }
            if (req.files['video']) {
                ...
                req.files['video'].forEach((f) => {
                    img.push(config.apiUrl + "/uploads/videos/" + req.userId + "/" + f.filename)
                })
                post['video'] = img;
            }
            //console.log(post);
            Feed.create(post, function(err, feed) {
                if (err) {
                    ...
                    res.status(200).json({
                        status: true,
                        message: "Your post is under review by moderator team.It will available soon.",
                        data: feed
                    });
                }
            });
...
router.get('/list', [verifyToken], function(req, res) {
    if(req.query.feed == "public"){
        Feed.find()
            ...
            .populate({
                path: "comments",
                populate: [
                    { path: 'user', select: '_id firstName lastName username profile_picture' }
                ]
            })
            .sort({ createdAt: 'desc' })
                    ...
                    User.find({ _id: { $in: userIdsToExclude } })
                        ...
                        .exec(function (err, users) {
                            if (err) {
                                ...
                                const allowedPosts = posts.filter(post => !users.some(user => user._id.equals(post.user._id)));
                ...
                if (err) {
                    ...
                    if (fr && fr.length > 0) {
                        for (var j = 0; j < fr.length; j++) {
                            ...
                            if (j == fr.length - 1) {
                                const newArray = friends.filter(element => !element.equals(req.userId));
                                cb(null, newArray);
                            }
                        }
                    } else {
    ...
    function getFollowers(friends, cb) {
        User.findOne({ _id: req.userId }, 'followings')
            .exec(function(e, user) {
                if (e) {
                    cb(e, null)
                } else {
                    if (friends.length > 0 && user.followings && user.followings.length > 0) {
            ...
            .exec(function(e, list) {
                if (e) {
                    ...
                    if (list.blocklist && list.blocklist.length > 0) {
                        var filtered=friends.filter(obj => !list.blocklist.includes(obj._id));
    ...
    function private(friends, blocklist, cb) {
        ...
        Feed.find({$and:[{ user: { $in: friends } },{user:{$nin:blocklist}}]})
...
router.get('/', [verifyToken], function(req, res) {
    ...
    function public(friends,blocklist, cb) {
        Feed.find()
            .where({ privacy: 'Public' })
            .where({$or:[{under_review:false},{$exists:{under_review:false}}]})
            .populate('user', '_id firstName lastName username profile_picture')
            .populate('likes', '_id firstName lastName username profile_picture')
            .populate({
                path: "comments",
            ...
            .exec(function(err, posts) {
                if (err) {
                    ...
                    var filtered=posts.filter(obj => !obj.user || !blocklist.includes(obj.user._id));
                                ...
                                if (follow) {
                                    Feed.find({ user: req.params.id })
                                        ...
                                        .populate({
                                            ...
                                            populate: [
                                                { path: 'user', select: '_id firstName lastName username profile_picture' },
                                                { path: 'likes', select: '_id firstName lastName username profile_picture' }
                                            ]
                                        })
                                        .exec(function(er, feed) {
                                            if (er) {
                ...
                .exec(function(er, fd) {
                    if (er) {
                        ...
                        if (fd.image && fd.image.length >= 1) {
                            ...
                            req.files.forEach((f) => {
                                req.body['image'].push(config.apiUrl + "/uploads/" + req.userId + "/" + f.filename);
                            })
                            Feed.findOneAndUpdate({ _id: req.params.id }, { $set: req.body }, { new: true },
                                function(err, post) {
                                    if (err) {
                                        res.status(500).json({ status: false, message: err, code: 500 });
                                    } else {
                            ...
                            req.files.forEach((f) => {
                                img.push(config.apiUrl + "/uploads/" + req.userId + "/" + f.filename);
                            })
                            req.body['image'] = img;
                            Feed.findOneAndUpdate({ _id: req.params.id }, { $set: req.body }, { new: true },
                                function(err, post) {
...
router.delete('/:id', [verifyToken], function(req, res) {
    ...
    Feed.deleteOne({ _id: req.params.id })
        .where({ user: req.userId }).exec(function(err, fd) {
            if (err) {
                res.status(500).json({ status: false, message: err, code: 500 });
            } else {
...
router.get('/like/:id', [verifyToken], function(req, res) {//like post
    Feed.findOne({ _id: req.params.id })
    //.where({under_review:false})
        ...
        .exec(function(err, post) {
            if (err) {
                ...
                if (post && post._id) {
                    ...
                    if (post.likes && post.likes.length > 0 && post.likes.find(x => x.equals(req.userId))) {
                    ...
                    res.status(400).json({ status: false, message: "Not allowed to like/unlike this post." });
                }
            }
        });
})
                ...
                if (p && p._id) {
                    Comment.create({
                        ...
                        if (er) {
                            ...
                            //console.log(c);
                            if (req.body.replyOf) {
                                Comment.updateOne({ _id: req.body.replyOf }, { $push: { replies: c._id } }, function(er, co) {
                                    if (er) {
                                        res.status(500).json({ status: false, message: er });
                                    } else {
                          ...
                          res.status(200).json({
                            ...
                            message: "Replied on a comment successfully",
                                ...
                                Feed.updateOne({ _id: req.params.id }, { $push: { 'comments': c._id } }, function(err, post) {
                                    if (err) {
                                        res.status(500).json({ status: false, message: err });
                                    } else {
                                        sendNotification(1,p,req.userId);
...
router.get('/report/:id',[verifyToken],function(req,res){
    ...
    Feed.updateOne({_id:req.params.id},{$push:{reported_by:req.userId}})
    .exec(function(err,post){
        if(err){
            console.log(err);
        }else{
...
var sendNotification=function(type,post,sender){
    User.findOne({_id:sender},'username')
    .exec(function(err,send){
        if(err){
            console.log(err);
        }else{
            if(type==0){
    ...
    User.findOne({ _id: recvr })
        ...
        .exec(function(err, dev) {
            if (err) {
                ...
                User.findOne({_id:sender},'_id username')
                .exec(function(e,s){
                    if(e){
                        console.log(e);
                    }else{
